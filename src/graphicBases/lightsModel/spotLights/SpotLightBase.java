package graphicBases.lightsModel.spotLights;
 ...
 * ,
 */
public class SpotLightBase extends SpotLight {
     ...
     * 实现类的命名方式为：光种+形状名+材质名+区别名,例如：SpotLight_cube_Gold_Pure
     *
     */
    public SpotLightBase(GLAutoDrawable glAutoDrawable, Vec3f position,Vec3f direction) {
        super(glAutoDrawable, position,direction);
    }
    public SpotLightBase(GLAutoDrawable glAutoDrawable, float x, float y, float z,Vec3f direction) {
        super(glAutoDrawable, x, y, z, direction);
    }

    /**
     * return float[]{cutOffAngle,outerCutOffAngle}
     *
     * @return
     */
    @Override
    protected float[] defineCutOff() {
        return new float[]{12.5f, 17.5f};
    }
